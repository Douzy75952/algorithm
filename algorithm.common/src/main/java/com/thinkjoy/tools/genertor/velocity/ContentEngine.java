package com.thinkjoy.tools.genertor.velocity;

import com.thinkjoy.base.exception.AppException;
...
public class ContentEngine {
	...
	public  void generatorVelocity(String[] args) {
		...
		try {
			...
			content.put("Object_package_name", args[8]);
			content.put("Mapper_package_name", args[7]);
			if (Integer.valueOf(args[0]) == 1) {

				content.toFile(args[5]  + args[4] + "Service" + ".java");
			}
			if (Integer.valueOf(args[0]) == 2) {
				content.toFile(args[5]  + args[4] + "ServiceSupport"
						+ ".java");
			}
			if (Integer.valueOf(args[0]) == 3) {
				content.toFile(args[5] + args[4] + "Mapper"
						+ ".java");
			}
